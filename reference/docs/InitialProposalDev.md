...
- Hybrid provider strategy: local LLMs via Ollama API or `llama.cpp` for offline/privacy-sensitive evaluations; cloud providers (OpenAI, Anthropic, OpenCode) for high-capability inference, routed through a provider-agnostic strategy pattern that selects the appropriate model based on audit pass, latency requirements, and privacy constraints.
...
- Audit history and comparison across notebook revisions.
...
| **AI / LLM Orchestration** | Ollama API, llama.cpp, OpenAI SDK, Anthropic SDK, OpenCode API | Provider-agnostic strategy pattern with unified `LLMProvider` interface. Supports `local` (Ollama/llama.cpp) and `cloud` (OpenAI, Anthropic, OpenCode) routing based on audit pass, privacy requirements, and model availability. Prompt templates from `NotebookBuildAudit.tex` (Levels 1–3) are parameterized and injected per audit pass. |
...
- **Strategy Pattern for LLM Providers:** A `LLMProvider` abstract base class defines `generate(prompt, **kwargs)`. Concrete implementations (`OllamaProvider`, `OpenAIProvider`, `AnthropicProvider`, `OpenCodeProvider`) handle provider-specific API formatting. The `LLMOrchestrator` service selects the appropriate provider at runtime based on the audit configuration.
...
The estimated time to complete this project is **11 weeks**, broken down into the following phases:
...
| **Phase 2 — FastAPI Backend & LLM Integration** | Implement FastAPI REST API with Pydantic models for audit sessions, notebook upload, and pass configuration. Build `LLMProvider` strategy pattern with Ollama, OpenAI, Anthropic, and OpenCode concrete implementations. Develop the `AuditStateMachine` implementing the six-pass protocol with gate decisions between Levels 1–3. Integrate `@jupyter-kit` for notebook parsing, cell execution, and output capture. Implement SSE streaming for real-time audit progress. | 4 weeks |
| **Phase 3 — Frontend Interface** | Scaffold Vite + React + TypeScript project. Build audit session dashboard (notebook upload, provider routing configuration, pass scope selection). Implement live audit progress view with per-pass risk score visualization. Create audit history browser with session comparison. Develop LLM provider configuration panel (local Ollama endpoint, cloud API keys). | 3 weeks |
| **Phase 4 — Integration & QA** | End-to-end integration testing with real notebooks across all six passes. Validate LLM prompt templates against reference outputs from `NotebookBuildAudit.tex`. Performance benchmarking for sandbox startup time and audit throughput. Security review of Docker sandbox isolation and API authentication. Final documentation and deployment guide. | 2 weeks |
...
| :--- | ---: | ---: |
| Developer Costs (2 devs × ~2.75 months × $1,000/mo) | $5,500 | ~21,450,000 COP |
| Hardware / Server PC | $1,000 | ~3,900,000 COP |
| **Total Project Cost** | **$6,500** | **~25,350,000 COP** |

**Payment Schedule:**
- 30% Upfront deposit to start development (~$1,950 USD / ~7,605,000 COP).
- 40% Upon completion of Phase 2 — Backend & LLM Integration (~$2,600 USD / ~10,140,000 COP).
- 30% Upon final delivery and deployment (~$1,950 USD / ~7,605,000 COP).
