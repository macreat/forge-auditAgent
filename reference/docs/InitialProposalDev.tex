...
\begin{enumerate}
  ...
  \item \textbf{Audit notebooks systematically} using the six-pass review protocol
    ...
    cloud (OpenAI, Anthropic, OpenCode) providers.
...
for offline/privacy-sensitive evaluations; cloud providers (OpenAI, Anthropic,
OpenCode) for high-capability inference, routed through a provider-agnostic strategy
...
\begin{itemize}
  ...
  \item Audit history and comparison across notebook revisions.
\end{itemize}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{l Y}
    ...
    Ollama API, \texttt{llama.cpp}, OpenAI SDK, Anthropic SDK, OpenCode API.
    Provider-agnostic strategy pattern with unified \texttt{LLMProvider} interface.
    Supports local (Ollama/llama.cpp) and cloud (OpenAI, Anthropic, OpenCode)
...
\begin{description}
  \item[Strategy Pattern for LLM Providers] A \texttt{LLMProvider} abstract base
    ...
    (\texttt{OllamaProvider}, \texttt{OpenAIProvider}, \texttt{AnthropicProvider},
    \texttt{OpenCodeProvider}) handle provider-specific API formatting. The
    \texttt{LLMOrchestrator} service selects the appropriate provider at runtime
    based on the audit configuration.
...
The estimated time to complete this project is \textbf{11 weeks}, broken down into
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{c Y c}
    ...
    Build \texttt{LLMProvider} strategy pattern with Ollama, OpenAI, Anthropic,
    and OpenCode concrete implementations. Develop the \texttt{AuditStateMachine}
    ...
    visualization. Create audit history browser with session comparison. Develop
    LLM provider configuration panel (local Ollama endpoint, cloud API keys). &
    3 weeks \\
    ...
    from \texttt{NotebookBuildAudit.tex}. Performance benchmarking for sandbox
    startup time and audit throughput. Security review of Docker sandbox isolation
...
\begin{table}[H]
  ...
  \begin{tabular}{l r r}
    ...
    \midrule
    Developer Costs (2 devs $\times$ $\approx$2.75 months $\times$
    \$1{,}000/mo) & \$5{,}500 & $\approx$ 21{,}450{,}000 COP \\
    Hardware / Server PC & \$1{,}000 & $\approx$ 3{,}900{,}000 COP \\
    \midrule
    \textbf{Total Project Cost} & \textbf{\$6{,}500} & \textbf{$\approx$
    25{,}350{,}000 COP} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{enumerate}
  \item \textbf{30\% Upfront deposit} to start development
    ($\approx$ \$1{,}950 USD / $\approx$ 7{,}605{,}000 COP).

  \item \textbf{40\% Upon completion of Phase 2} --- Backend \& LLM Integration
    ($\approx$ \$2{,}600 USD / $\approx$ 10{,}140{,}000 COP).
